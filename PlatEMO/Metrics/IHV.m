function score = IHV(Population,optimum)
% <max>
% Hypervolume
...
%--------------------------------------------------------------------------
% same as HV but with fmin and fmax interchanged so that the reference
% point is created based on the current population and the optimum is
% compared to it
    ...
    if size(PopObj,2) ~= size(optimum,2)
        ...
        [N,M]  = size(PopObj);
        fmin   = max(optimum,[],1);
        fmax   = min(min(PopObj,[],1),zeros(1,M));
